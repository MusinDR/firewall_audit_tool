...
def run_audit_and_export(layer_name: str, audit_checks: dict, output_path: str = "tmp/audit_findings.csv") -> tuple[int, str]:
...
def export_findings_to_csv(findings: list[dict], filepath: str):
    ...
    with open(filepath, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Severity", "Issues", "Layer", "Rule #", "Rule Name",
        ...
        for uid, issues in grouped.items():
            ...
            max_severity = max((i["severity"] for i in issues), key=lambda s: SEVERITY_ORDER.get(s, 0))
